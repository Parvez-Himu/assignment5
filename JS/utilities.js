...
// adding history-tab function

function createDataForHistory(inputValidAmount, donationInfo) {
    if (typeof (inputValidAmount) === 'number') {
        num += 1;
        const transaction = (Math.random() * 10).toFixed(5);
        const date = new Date();
        const donationTitle = donationInfo.innerText;
        const historyContainer = document.getElementById('history-container');

        // Create the div that will hold the new history entry
        const div = document.createElement('div');
        div.innerHTML = ` 
            <div class="w-7 h-7 p-2 bg-gray-300 rounded-sm flex justify-center items-center mb-1">${num}</div>
        ...
        // Append the new history entry at the top
